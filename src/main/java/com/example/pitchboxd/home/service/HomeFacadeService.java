...
public class HomeFacadeService {
    ...
    public HomeResponses getHomeData(MatchFilter state, Long seasonId) {
        ...
        for (MatchSummary matchSummary : matchSummaries) {
            List<HotReviewSummary> topReviews = matchReviewQueryService.getTopHotReviewsByMatchId(matchSummary.id(), 3);
            List<HotReviewResponse> hotReviewResponses = topReviews.stream()
